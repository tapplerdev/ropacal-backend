...
func authorizeSubscription(db *orgdb.DB, userID string, channel string) (bool, error) {
	...
	case "company":
		...
		// the shared channel" from "someone sent us a malformed channel name".
		// A stale client sees a 403 on its next resubscribe, which is the loud
		// signal Deploy 3b then waits on.
		if ch.legacy {
