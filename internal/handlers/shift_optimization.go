...
func ReoptimizeActiveShift(db *sqlx.DB, redisClient *redis.Client, shiftID string, centrifugoClient *centrifugo.Client, skipGates bool) error {
	...
	// Step 3: Hard-delete incomplete warehouse_stop tasks (AFTER gate check passes).
	// They're system-generated routing artifacts the optimizer regenerates below, so
	// soft-deleting them only spammed the task-history audit view with phantom "removed
	// warehouse stop" rows on every reopt. Completed ones (driver actually visited) are kept.
	result, err := db.Exec(`
		DELETE FROM route_tasks
		WHERE shift_id = $1 AND task_type = 'warehouse_stop' AND is_completed = 0 AND is_deleted = false
	`, shiftID)
	if err != nil {
		log.Printf("⚠️  [REOPTIMIZE] Failed to delete warehouse stops: %v", err)
	} else {
	...
	// Filter out warehouse_stop tasks from our working slice
